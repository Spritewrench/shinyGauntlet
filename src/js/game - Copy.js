...
(function() {
  ...
  Game.prototype = {
    ...
    //  this.player.shield.width = this.player.shield.width/2;
     // this.player.shield.height = this.player.shield.height/2;
...
//        dist = Math.sqrt(tx*tx+ty*ty);        
        ...
        for(var i = 0; i < this.monster.length; i++){
          if(this.monster[i].attackCD > 0 && this.monster[i].visible == true && ((this.monster[i].monType > 5 && this.monster[i].monType < 22 || this.monster[i].monType == 99) || this.monster[i].monType == 20 || this.monster[i].monType == 22)){
            var flicker = this.game.rnd.integerInRange(-1,2);
            var size = 150;
            ...
            this.shadowTexture.context.beginPath();
            this.shadowTexture.context.fillStyle = 'rgba(255, 255, 255, 1)';
            this.shadowTexture.context.arc(this.monster[i].x,this.monster[i].y,size-50 + flicker, 2 * Math.PI, false);
            this.shadowTexture.context.fill();            
          }
      ...
      //close doors
      console.log(world[this.currentMap].cleared);
      var monAlive = 0;
      for(var i = 0; i < this.monster.length;i++){
      ...
      for(var i = 0; i < this.monster.length;i++){
        ...
        if(this.monster[i].attackCD < 0 && this.monster[i].monType <= 22 && this.monster[i].monType >= 21 && dist <= (this.lightSize+50)){
        ...
        if (this.monster[i].visible){
          ...
          if(this.monster[i].monType == 3 && (this.monster[i].attackCD == 100 || (this.monster[i].attackCD == 100 && this.monster[i].hp <= 30)) && this.monster[i].knockback <= 0 ){
            ...
            if(this.monster[i].monType == 4 && this.monster[i].attackCD > 75 &&  this.monster[i].knockback <= 0 && this.player.alpha == 1){
              this.spawn(this.monster.length,12,11,'lazer',this.monster[i].x,this.monster[i].y,16,3,0,2);
    ...
    playerHit: function (obj1, obj2) {  
      ...
      if(obj2.monType > 5 &&  obj2.monType < 10){
        ...
        switch(this.player.wep.prefix){
            ...
            case 3:
              ...
              this.lightConstant++;
              if(this.lightConstant > 6){
                this.lightConstant = 6;
              }
              break;
            case 2:
              this.txt.setText("Passive: Take half damage on all attacks \n Active: \"Shield\" Block an attack");
      ...
      //life
      if(obj2.monType == 20 && this.player.hp < this.player.maxHp){
    ...
    reload: function () {
          ...
          if(this.currentMap == winPos){
            ...
            if(world[this.currentMap].mon[i].monType == 0){
              ...
              if(world[this.currentMap].mon[i].prefix > 1 && world[this.currentMap].mon[i].prefix < 6){
                    ...
                    this.spawn(this.monster.length,22,randomizer,'scroll',x,y,32,3,0,2);
                    this.textCounter = 200;
      ...
      //bats (mundane mobs)
      if(this.currentMap != winPos){
        var mobNum = Math.floor((Math.random()*3)+1);
        for(var j =0; j < mobNum; j++){
          x = Math.floor((Math.random()*600)+100);
          y = Math.floor((Math.random()*400)+100); 
          var speed = Math.floor((Math.random()*2)+1); 
          this.spawn(this.monster.length,21,11,'trap',x,y,32,1,0,1);    
          this.monster[this.monster.length-1].attackCD = -1;  
        }             
      }
